...
class Poly
{
	public:
		...
		~ Poly()
		{
/*			list<Poly_node>::const_iterator first;
			Poly_node* last = new Poly_node;
			first = this->node.begin();
			for( ; first != node.end();)	//if first == NULL break
			{
				last = node.begin();	//let last point to the same node as first node
				first ++;	//let the first pointer points to next pointer
				delete last;	//delete the current node
			}
*/		}
