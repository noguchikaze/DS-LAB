...
int main(int argc, char* argv[])
{
    ...
    for(int i = 1; i < y-1; i++)//1到y-2才是graph的範圍
    {
        ...
        {//檢查4個方向 看看有沒有edge存在 預設都不存在
            if(map[i][j].node == 0)//如果現在的vertex存在
            {
                ...
                {  
					map[i][j].edge[E]=1;    edge_total++;   
				}    //edge存在
                if(map[i][j-1].node == 0)    //左邊 x-1 可以走的話
                {   
					map[i][j].edge[W]=1;    edge_total++;    
				}    //edge存在
                if(map[i+1][j].node == 0)    //上面 y+1 可以走的話
                {   map[i][j].edge[N]=1;    edge_total++;
			    }   //edge 存在
                if(map[i-1][j].node == 0)    //下面 y-1 可以走的話
                {   map[i][j].edge[S]=1;    edge_total++;
			    }   //edge存在
            }//end of if
        }//end of for
    }//end of for
    ...
    {//已經沒路可走 換個方向 而起始條件已經先弄好
        ...
        {   //先把回頭的路線丟進queue裡面 dir紀錄從上個點走來的方向
			...
			{
				//cout<<"stack is empty-> break"<<endl;
				break;	
			}	//跳出迴圈	
       ...
       while(direction < 3)//還有方向或路需要確認是否可以走
       {
            ...
            else if(map[col+move[direction].ver][row+move[direction].hor].node == 0) //如果想走的edge存在
            {   
    ...
    {//注意 因為y軸標示跟題目相反 要改一下
		...
		route.pop();
        fout << tmp.x << ' ' << (y-1-tmp.y) << endl;
    }
	fout<<x_cor<<' '<<y-1-y_cor<<endl;	//起點
