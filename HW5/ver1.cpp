...
int main(int argc, char* argv[])
{
    ...
    for(int i = 1; i < y-1; i++)//1到y-2才是graph的範圍
    {
        ...
        {//檢查4個方向 看看有沒有edge存在 預設都不存在
            if(map[i][j] == 0)//如果現在的vertex存在
            {
                x_cor = j;  y_cor = i;
                if(map[i][j+1] == 0)   //右邊 x+1 可以走的話
                {   map[i][j].edge[E]=1;    edge_total++;   }    //edge存在
                if(map[i][j-1] == 0)    //左邊 x-1 可以走的話
                {   map[i][j].edge[W]=1;    edge_total++;    }    //edge存在
                if(map[i+1][j] == 0)    //上面 y+1 可以走的話
                {   map[i][j].edge[N]=1;    edge_total++;    }   //edge 存在
                if(map[i-1][j] == 0)    //下面 y-1 可以走的話
    ...
    {//已經沒路可走 換個方向 而起始條件已經先弄好
        ...
        {   //如果上一步已經試到最後一個方向了
            //先把上一步的值丟進queue裡面
            route.push(Graph(last_move.y, last_move.x, last_move.dir));
